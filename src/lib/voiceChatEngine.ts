...
async function loadVoiceStyle(voiceStylePaths: string[]) {
  const ort = await import('onnxruntime-web/all');
...
async function loadTextToSpeech(onnxDir: string, sessionOptions: any = {}) {
  const ort = await import('onnxruntime-web/all');
  
  ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.23.2/dist/';
  ort.env.wasm.numThreads = 1;
  ort.env.logLevel = 'error'; // Suppress warnings, only show errors
